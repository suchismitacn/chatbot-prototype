...
class SendingMiddleware implements Sending
{
    ...
    protected function processPayload($payload, $bot) {
        ...
        if ($message instanceof OutgoingMessage) {
            ...
            $this->storeConversation('chatbot', $message->getText(), $message->getActions());
        }
    }

    protected function storeConversation(string $origin, string $text, array $options = null)
    {
        return (new ConversationRepository)->storeConversation(null, request()->session()->getId(), $origin, $text, $options);
    }
}
